...
The rule sections are : `Marker grammar`, `Oracle commands`, `Grading` for each mode, `Done condition`, `Anti-cheating`, and the three scoring facts of `Persistence` under `Scoring and stopping`. 
...
- `Reference — original upstream source`, `Checkpointing`, and `Previous iteration feedback`, each only when the run has one: prefetched upstream material, a chunked run, or a repeated pass.
...
`prepare` also accepts the run's shape: `--chunk-minutes` states a chunked attempt, `--previous-feedback FILE` with `--iteration-index/--iteration-total` states a repeated pass, and `--source-context full` ships a prefetched `upstream_source/` as reference material. Each one is stated in both Vero's `INSTRUCTION.md` and `MINIVERO_TASK.md`; without them a run is a single pass.
