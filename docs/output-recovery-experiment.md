# Frozen Vero comparison: complete instructions and recoverable output
...
This report retains the original joint experiment at `b60f044`, alongside the combined
task-delivery and output-recovery change. The experiment used the old CAS workspace backend;
it is not fresh model sampling of the current restic-based implementation or the subsequently
revised recovery notice. No effect in the tables below can be attributed to either individual
feature. Current implementation checks are recorded in the [review map](output-recovery-review.md).
...
support commit [`9727039`](https://github.com/pku-xht/alaya/tree/9727039) has identical
executable Lean sources, old smoke/pilot runners, and toolchain files to that control;
its documentation/history excludes
private prior runs.
...
experiment revision, not to every later revision. The local
[frozen validation metadata](output-recovery-experiment-validation.json) preserves the
frozen-experiment record without changing its fields or values. See also the
[original validation metadata](https://github.com/pku-xht/alaya/blob/b60f044badb448765d4e0d7ecfd9843452c65a34/docs/output-recovery-validation.json) and the
...
The combined implementation has separate [validation](output-recovery-review.md); this report
does not claim a fresh model rerun of that implementation or its revised notice.
...
there are no real-model page results to present as a successful recovery. Tests in the frozen
experiment revision separately recovered the exact omitted middle, concatenated Unicode/long-line
pages to EOF, and recovered after forks, reopening CAS, GC, resume, and container recreation.
...
grading leaves. This report publishes only the reviewed aggregate results and frozen
validation metadata; it includes no raw requests, responses, CAS/cache, or private run files.
The machine-readable aggregate metrics and Vero reproduction harness are linked from the
frozen experiment branch. The HTML context view remains a
...
Validation of the current combined implementation is recorded separately in the
[review map](output-recovery-review.md). Including this report does not rerun a model
or revalidate the current implementation.
