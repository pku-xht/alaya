...
The new frozen experiment binary is byte-identical to the final local build.
All 42 Lean/build source files in the public candidate match the validated Linux
tree. See [validation metadata](output-recovery-validation.json) and the
[machine-readable comparison](output-recovery-experiment-result.json).
...
Use [vero-smoke.md](vero-smoke.md), [vero-experiment.md](vero-experiment.md), and
`example/compare_vero_runs.py --help`. Build the old support commit and new commit
...
grading leaves. Only reviewed aggregate metrics, hashes, implementation, and
reproduction scripts are included in the PR. The HTML context view remains a
prospective reconstruction; the saved actual request bodies establish transmission.

Validation: `lake build`; **130 Lean tests passed with no skips** on a Linux
