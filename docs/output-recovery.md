...
that a provider request was actually sent. The Vero runners use the file-delivery path for new
runs; their commands and evidence are documented in [vero-smoke.md](vero-smoke.md) and
[vero-experiment.md](vero-experiment.md).
...
| `warning` | Explains the reference, offset convention, and continuation procedure |
...
Call again with `offset` set to `next_offset` until `eof` is true and `next_offset` is null.
